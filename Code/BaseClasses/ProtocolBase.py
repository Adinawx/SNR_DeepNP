...
class ProtocolBase(Data):
    ...
    def load_data_protocol(self):
        ...
        rep = self.cfg.protocol.rep
        self.sinr_vecs = self.test_sinr[:rep, :] #### HERE CHOOSE BETWEEN VAL AND TEST
    ...
    def protocol_run(self, rep_seq=None):
        ...
        for r in rep_seq:
            ...
            ###################### for gini prediction #######################################################################################
            if self.cfg.protocol.pred_type == 'gini' and self.cfg.model.test_type == 'Par':
                th_update = self.cfg.data.future - self.cfg.protocol.rtt
                T = self.cfg.protocol.T
                block_number = int(T / th_update)
                all_erasures = torch.zeros(cur_sinr.shape[0])
                all_th = torch.zeros(cur_sinr.shape[0])
                ...
                for ind_b in range(block_number):
                    ...
                    for ind_th in range(len(self.cfg.data.sinr_threshold_list)):
                        block[ind_th, :] = (cur_sinr[ind_b*th_update: (ind_b+1)*th_update] > self.cfg.data.sinr_threshold_list[ind_th]).float()
            ...
            for t in range(T):
                ...
                if t >= int(rtt/2):  # FB arrives
                    ...
                    if self.cfg.protocol.pred_type == 'gini' and self.cfg.model.test_type == 'Par':
                        erasure_pred, th = self.get_pred(fb, all_th, t=t, cur_erasure_vec=all_erasures, th=th)  # gets erasures input
    ...
    def plot_channel_rate(self, r_plt=0):
        ...
        cr_pred = torch.sum(self.preds[r_plt, :T - future, :rtt].detach(), dim=-1)
        cr_true = torch.sum(self.true_erasure_pred[r_plt, :T - future, :rtt], dim=1)
